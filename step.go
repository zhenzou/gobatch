...
func (step *simpleStep) Exec(ctx context.Context, execution *StepExecution) (err BatchError) {
	...
	for {
		...
		_logger.Error(ctx, "step execute will continue, jobExecutionId:%v, stepName:%v", execution.JobExecution.JobExecutionId, execution.StepName)
	}
	if be != nil {
		_logger.Error(ctx, "step execute failed, jobExecutionId:%v, stepName:%v, err:%v", execution.JobExecution.JobExecutionId, execution.StepName, be)
	} else {
		_logger.Info(ctx, "step execute completed, jobExecutionId:%v, stepName:%v", execution.JobExecution.JobExecutionId, execution.StepName)
	}
	execution.finish(be)
	for _, listener := range step.listeners {
	...
	_logger.Info(ctx, "step execute finish, jobExecutionId:%v, stepName:%v, stepStatus:%v", execution.JobExecution.JobExecutionId, execution.StepName, execution.StepStatus)
	return nil
}
...
func (step *chunkStep) Exec(ctx context.Context, execution *StepExecution) (err BatchError) {
	...
	if err != nil {
		_logger.Error(ctx, "open resource failed, jobExecutionId:%v, stepName:%v, err:%v", execution.JobExecution.JobExecutionId, execution.StepName, err)
		execution.finish(err)
		return err
	}
	defer func() {
		...
		if err != nil {
			_logger.Error(ctx, "close resource failed, jobExecutionId:%v, stepName:%v, err:%v", execution.JobExecution.JobExecutionId, execution.StepName, err)
		}
	}()
	input := newChunk()
	output := newChunk()
	for !input.end {
		...
		if txErr != nil {
			_logger.Error(ctx, "start transaction err, jobExecutionId:%v, stepName:%v, err:%v", execution.JobExecution.JobExecutionId, execution.StepName, txErr)
			err = txErr
			break
		}
		...
		if err != nil {
			_logger.Error(ctx, "doChunk err, jobExecutionId:%v, stepName:%v, err:%v", execution.JobExecution.JobExecutionId, execution.StepName, err)
			txErr = txManager.Rollback(tx)
			if txErr != nil {
				_logger.Error(ctx, "rollback transaction err, jobExecutionId:%v, stepName:%v, err:%v", execution.JobExecution.JobExecutionId, execution.StepName, txErr)
			}
			break
		}
		txErr = txManager.Commit(tx)
		if txErr != nil {
			_logger.Error(ctx, "commit transaction err, jobExecutionId:%v, stepName:%v, err:%v", execution.JobExecution.JobExecutionId, execution.StepName, txErr)
...
func (step *chunkStep) doChunk(ctx context.Context, chunk *ChunkContext, input *chunk, output *chunk) (err BatchError) {
	defer func() {
		if er := recover(); er != nil {
			_logger.Error(ctx, "panic on chunk executing, jobExecutionId:%v, stepName:%v, err:%v, stack:%v", chunk.StepExecution.JobExecution.JobExecutionId, chunk.StepExecution.StepName, er, string(debug.Stack()))
	...
	_logger.Debug(ctx, "doChunk start, jobExecutionId:%v, stepName:%v", chunk.StepExecution.JobExecution.JobExecutionId, chunk.StepExecution.StepName)
	for _, listener := range step.chunkListeners {
		err = listener.BeforeChunk(chunk)
		if err != nil {
			_logger.Error(ctx, "chunk listener executing error, jobExecutionId:%v, stepName:%v, listener:%v, err:%v", chunk.StepExecution.JobExecution.JobExecutionId, chunk.StepExecution.StepName, reflect.TypeOf(listener).String(), err)
			return err
		}
	}
	err = readChunk(step.reader, chunk, input, step.chunkSize)
	if err != nil {
		_logger.Error(ctx, "read chunk data error, jobExecutionId:%v, stepName:%v, err:%v", chunk.StepExecution.JobExecution.JobExecutionId, chunk.StepExecution.StepName, err)
		for _, listener := range step.chunkListeners {
			listener.OnError(chunk, err)
		}
		return err
	}
	_logger.Debug(ctx, "read chunk data success, jobExecutionId:%v, stepName:%v, read count:%v", chunk.StepExecution.JobExecution.JobExecutionId, chunk.StepExecution.StepName, len(input.items))
	...
	if len(input.items) > 0 {
		for index, item := range input.items {
			...
			if step.processor != nil {
				...
				if err != nil {
					_logger.Error(ctx, "process chunk item error, jobExecutionId:%v, stepName:%v, item:%v, err:%v", chunk.StepExecution.JobExecution.JobExecutionId, chunk.StepExecution.StepName, item, err)
					for _, listener := range step.chunkListeners {
						listener.OnError(chunk, err)
					}
					return err
				}
				_logger.Debug(ctx, "process chunk item success, jobExecutionId:%v, stepName:%v, inItem:%v, outItem:%v", chunk.StepExecution.JobExecution.JobExecutionId, chunk.StepExecution.StepName, item, outItem)
			}
			if outItem != nil {
		...
		if len(output.items) > 0 && step.writer != nil {
			...
			if err != nil {
				_logger.Error(ctx, "write chunk data error, jobExecutionId:%v, stepName:%v, err:%v", chunk.StepExecution.JobExecution.JobExecutionId, chunk.StepExecution.StepName, err)
				for _, listener := range step.chunkListeners {
					listener.OnError(chunk, err)
				}
				return err
			}
			_logger.Debug(ctx, "write chunk data success, jobExecutionId:%v, stepName:%v, write count:%v", chunk.StepExecution.JobExecution.JobExecutionId, chunk.StepExecution.StepName, len(output.items))
		}
	}
	for _, listener := range step.chunkListeners {
...
func (step *partitionStep) Exec(ctx context.Context, execution *StepExecution) (err BatchError) {
	...
	for _, listener := range step.partitionListeners {
		...
		if err != nil {
			_logger.Error(ctx, "partition listener executing error, jobExecutionId:%v, stepName:%v, listener:%v, err:%v", execution.JobExecution.JobExecutionId, execution.StepName, reflect.TypeOf(listener).String(), err)
			return err
		}
	}
	subExecutions, err := step.split(ctx, execution, step.partitions)
	if err != nil {
		_logger.Error(ctx, "step split error, jobExecutionId:%v, stepName:%v", execution.JobExecution.JobExecutionId, execution.StepName, err)
		for _, listener := range step.partitionListeners {
	...
	for _, subExecution := range subExecutions {
		task := func(e *StepExecution) func() (interface{}, error) {
			return func() (interface{}, error) {
				_logger.Info(ctx, "sub-step execute start, jobExecutionId:%v, sub-step name:%v", e.JobExecution.JobExecutionId, e.StepName)
	...
	_logger.Info(ctx, "finish step execution, jobExecutionId:%v, stepName:%v, stepStatus:%v", execution.JobExecution.JobExecutionId, execution.StepName, execution.StepStatus)
	return nil
}
...
func (step *partitionStep) split(ctx context.Context, execution *StepExecution, partitions uint) ([]*StepExecution, BatchError) {
	if execution.StepExecutionContext.Exists(partitionStepPartitionsKey) {
		...
		_logger.Info(ctx, "split step:%v, saved partitions in StepExecutionContext:%v, try to get last splitted subExecutions", execution.StepName, savedPartitions)
